...
$aCategories = get_the_category($oPost->ID);

if (!empty($aCategories)) {
    $cateImg = FunctionHelper::getTermFeaturedImage($aCategories[0]->term_id);
    $cateName = $aCategories[0]->name;
} else {
    $cateImg = FunctionHelper::getPlaceholderImageUrl();
    $cateName = '';
}
...
if (defined('HSBLOG2_SC_PREFIX')) {
    ...
    echo App::get('UniqueVerticalBoxItemSc')->renderSc([
        ...
        'author_avatar'   => $cateImg,
        'author_name'     => $cateName,
    ]);
    return;
}
