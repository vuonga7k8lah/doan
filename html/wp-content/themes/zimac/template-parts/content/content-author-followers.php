...
$aQuery = FollowerModel::getAUserFollowers($userID, [
    'number'    => $number,
    'offset'    => $offset,
    'paged'     => $paged
]);
$aUserFollowers = $aQuery['data'];
$totalItems =  $aQuery['totalItems'];
$totalPages = ceil($totalItems / $number);
?>

<?php if (empty($aUserFollowers)) : ?>
...
<?php if (!empty($aUserFollowers)) : ?>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 xl:gap-8 mb-13">
        <?php foreach ($aUserFollowers as $oUser) : ?>
...
<?php if ($totalItems > count($aUserFollowers)) : ?>
