...
abstract class AUpload
{
    ...
    public function setImageSource(string $source): AUpload
    {
        if (!in_array($source, $this->_aValidSources)) {
            throw new RuntimeException(esc_html__('Invalid source', 'myshopkit-popup-smartbar-slidein'));
        }
    ...
    public function setUpdateAttachmentId(int $_updateAttachmentId): AUpload
    {
        if ($this->_isAttachmentAuthor($_updateAttachmentId)) {
            ...
            throw new Exception(esc_html__('You are not author of this attachment', 'myshopkit-popup-smartbar-slidein'));
        }
    ...
    public function setType(string $type): AUpload
    {
        if (in_array($type, $this->aValidTypes)) {
            ...
            throw new RuntimeException(sprintf(esc_html__('Invalid image type %s', 'myshopkit-popup-smartbar-slidein'), $type));
        }
    ...
    protected function _validateFileSize($fileSize): bool
    {
        if ($fileSize > wp_max_upload_size()) {
            throw new Exception(sprintf(esc_html__('The file is large than expected %sM', 'myshopkit-popup-smartbar-slidein'),
    ...
    protected function _validateExtension($fileMine): bool
    {
        if (!in_array($fileMine, $this->aExtensions)) {
            throw new Exception(sprintf(esc_html__('The file type %s is not allowed', 'myshopkit-popup-smartbar-slidein'),
                $fileMine));
        }
    ...
    protected function _proceedUpdateOrInsertAttachment(): array
    {
        if (empty($this->_fileMimeType)) {
            throw new Exception(esc_html__('The file mime type is required', 'myshopkit-popup-smartbar-slidein'));
        }

        if (empty($this->_fileName)) {
            throw new Exception(esc_html__('The file name type is required', 'myshopkit-popup-smartbar-slidein'));
        }

        if (empty($this->_extension)) {
            throw new Exception(esc_html__('The extension is required', 'myshopkit-popup-smartbar-slidein'));
        }

        if (!in_array($this->_imgSource, $this->_aValidSources)) {
            throw new Exception(esc_html__('The image source is not allowed', 'myshopkit-popup-smartbar-slidein'));
        }
        ...
        if (!empty($this->_updateAttachmentId)) {
            ...
            if (!$status) {
                throw new Exception(esc_html__('We could not update the data', 'myshopkit-popup-smartbar-slidein'));
            }
        ...
        return [
            'id'     => $attachmentId,
            'thumbnails' => $this->getThumbnailDefault($attachmentId),
            'url'    => wp_get_attachment_image_url($attachmentId, 'full'),
            'msg'    => sprintf(
                esc_html__('The file %s has been uploaded successfully', 'myshopkit-popup-smartbar-slidein'),
                $this->_fileName
            ),
            'status' => 'success'
        ];
