...
class WPUpload extends AUpload
{
            ...
            if (!$hasOneCorrectAtLeast) {
                return MessageFactory::factory('normal')
                    ->error(esc_html__('Somethings went wrong, We could not upload your files',
                        'myshopkit-popup-smartbar-slidein'), 422);
            }
    ...
    private function _prepareImage(): AUpload
    {
        if (
            ...
            throw new RuntimeException(esc_html__('Invalid parameter', 'myshopkit-popup-smartbar-slidein'));
        }
        ...
        switch ($this->singularFile['error']) {
            ...
            case UPLOAD_ERR_NO_FILE:
                throw new RuntimeException(esc_html__('No file sent.', 'myshopkit-popup-smartbar-slidein'));
            case UPLOAD_ERR_INI_SIZE:
            case UPLOAD_ERR_FORM_SIZE:
                throw new RuntimeException(esc_html__('Exceeded filesize limit.', 'myshopkit-popup-smartbar-slidein'));
            default:
                throw new RuntimeException(esc_html__('Unknown errors.', 'myshopkit-popup-smartbar-slidein'));
        }
    ...
    protected function _processUpload(array $aFile): array
    {
        ...
        if (!isset($aFile['tmp_name']) || empty($aFile['tmp_name'])) {
            return [
                'status' => 'error',
                'msg'    => esc_html__('Sorry, We could not download this file.', 'myshopkit-popup-smartbar-slidein')
            ];
        }
        ...
        if (!$status) {
            return [
                'status' => 'error',
                'msg'    => esc_html__('Sorry, We could not copy this file.', 'myshopkit-popup-smartbar-slidein')
            ];
        }
